...
, mSettingsText()
{
	...
	mSettingsText.setFont(mFonts.get(Fonts::Main));
	mSettingsText.setPosition(300.f, 50.f);
	mSettingsText.setCharacterSize(80u);
	mSettingsText.setString("ASTEROIDS \n   Settings");
	...
	backButton->setPosition((context.window->getSize().x / 2) - 100, 620.f);
...
void SettingsState::addButtonLabel(Player::Action action, float y, const std::string& text, Context context)
{
	...
	mBindingButtons[action]->setPosition((context.window->getSize().x / 2)- 100, y);
	mBindingButtons[action]->setText(text);
	mBindingButtons[action]->setToggle(true);

	mBindingLabels[action] = std::make_shared<GUI::Label>("", *context.fonts);
	mBindingLabels[action]->setPosition(context.window->getSize().x / 2 + 125.0f, y + 15.f);
